/* nogarbage module for pojeby
 ...
 Thanks to the following people for testing: Aretino, matrix, nerd
*/
        ...
        for(int i=0; i<NG_MAX_PATTERNS; i++)
        {
            if(!pattern[i].isDefault())
            {
                if((error=regcomp(&preg, pattern[i], REG_ICASE | REG_EXTENDED))!=0)
                {
                    ...
                    net.send(HAS_N, "[!] Invalid %s pattern '%s' (%s)", name.c_str(), (const char*) pattern[i], error_str);
                    regfree(&preg);
                    continue;
                }

		ret=regexec(&preg, msg, 1, &pmatch, 0);
		regfree(&preg);
...
void chanlistCustomData::stats(const char *pl_user)
{
  ptrlist<entry>::iterator i;

  for(i=data.begin(); i; i++)
    net.sendUser(pl_user, "%s (%s@%s)", i->nick, i->ident, i->host);

  net.sendUser(pl_user, "total %d users", data.entries());
}
...
void ng_action(chanuser *, chan *, ng_options *);
void ng_ban(chan *, chanuser *, ng_options *);
void ng_kick(chan *, chanuser *, ng_options *);
int ng_calc_caps(const char *);
void ng_handle_query_spam(const char *, const char *);
void ng_debug(const char *);

time_t ng_next_save;
int ng_current_cycle_chan;
...
void ng_save_conf()
{
    ...
    {
        net.send(HAS_N, "[-] cannot open %s for writing: %s", (const char*) MY_CONF, strerror(errno));
   ...
   fclose(fh);
   ng_next_save=0;
   net.send(HAS_N, "[*] Autosaving nogarbabe config"); // m->name
}
        ...
        case 1 : if(!(((chanuserCustomData*)cu->customData(m->desc))->info[opt->type].level&NG_WARNED))
                 {
                     ...
                     ME.notice(cu->nick, "%s", (const char*) opt->WARNMSG);
                 }
...
void ng_ban(chan *ch, chanuser *cu, ng_options *opt)
{
    ...
    if(!set.BOTS_CAN_ADD_BANS
       || !protmodelist::addBan(ch->name, str1.c_str(), "nogarbage", opt->BANTIME, str2.c_str()))
    {
...
void ng_debug(const char *pl_user)
{
  CHANLIST *cl;

  foreachNamedChanlist(cl) {
    net.sendUser(pl_user, "%s:", (const char*) cl->name);
    ((chanlistCustomData*)cl->customData(m->desc))->stats(pl_user);
    net.sendUser(pl_user, " ");
  }
}
...
{
    m=new module("nogarbage", "patrick <patrick@psotnic.com>", "0.1-pre4+quickfix");
